...
pub fn deposit(ctx: &mut Context<Deposit>, amount: u64, max_x: u64, max_y: u64) -> Result<()> {
...
fn deposit_token(ctx: &Context<Deposit>, is_x: bool, amount: u64) -> Result<()> {
    ...
    let (from, to) = match is_x {
        true => (ctx.user_x.to_account_info(), ctx.vault_x.to_account_info()),
        false => (ctx.user_y.to_account_info(), ctx.vault_y.to_account_info()),
    };
